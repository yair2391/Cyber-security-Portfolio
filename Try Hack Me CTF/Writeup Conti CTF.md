...
###### **Q9: What is the command line that executed this web shell?**
  ...
  ביצעתי התאמה וחיפוש ישיר בלוגים תוך מיקוד בשדה ה-`CommandLine` (או שדה הבקשה המתאים בלוגי ה-IIS) וסינון לפי שם קובץ ה-סקריפט (`aspx`), בעזרת השאילתה:
...
###### **Q10: What three CVEs did this exploit leverage? Provide the answer in ascending order.**
  ...
  1. **CVE-2018-13374** – חולשת Path Traversal / Arbitrary File Read במוצרי Fortinet (FortiOS SSL VPN), המאפשרת לתוקפים לקרוא קבצים רגישים ופרטי התחברות ללא אימות מוקדם.
  2. **CVE-2018-13379** – חולשת ניהול סשנים או שינוי סיסמאות במוצרי Fortinet, המשמשת לצד חולשות VPN כדי להשיג שליטה או לגנוב גישות משתמשים בלתי מורשות.
  3. **CVE-2020-0796** – חולשת קריטית לביצוע קוד מרחוק (RCE) המכונה **"SMBGhost"**, הקיימת בפרוטוקול התקשורת SMBv3 של מערכות ההפעלה Windows, ומאפשרת לתוקפים להריץ קוד ברמת הליבה (Kernel) של השרת.